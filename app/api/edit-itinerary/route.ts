...
import {
  extendItinerarySystemPrompt,
  extendItineraryUserPrompt,
} from "../prompts";
...
export async function POST(request: NextRequest) {
  ...
  const { existingItinerary, userInput, preferences, tripId, tripName } =
    validation.data;

  const system = extendItinerarySystemPrompt();
  const userPrompt = extendItineraryUserPrompt(
    existingItinerary,
    preferences,
    userInput
  );
  ...
  const functions: ZodFunctionDef[] = [
    {
      ...
      description:
        "Updates an existing travel itinerary based on user input and preferences",
  ...
  if (!modelOutput) {
    return errorResponse(
      "Failed to update travel itinerary due to AI model failure.",
      500
    );
  }

  const newItinerary = normalizeNewItinerary(
    modelOutput.newItinerary,
    tripId,
    tripName
  );
...
function normalizeNewItinerary(
  newItinerary: any[],
  tripId: string,
  tripName: string
): any[] {
